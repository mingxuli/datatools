...
`sphinx <http://sphinx.pocoo.org/>`_ 网站的页面要比Sphinx默认的页面好看，那么我们就来尝试建立这样的网站吧.如果你已经
下载了本教程的源代码(see :ref:`fetching-the-data`)，请把 :file:`_static/default.css` 和 :file:`_static/logo.png` 两个文件拷贝到你的_static目录下,把 :file:`_templates/layout.html` 拷贝到_templates目录下。

如果你没有下载，可以参考下面的代码，自己进行创建。其中default.css的代码为::
...
这三个文件创建后，Sphinx能够自动的加载，执行 ``make html`` 命令再编译一遍吧，修改后的效果如下：
...
5) conf.py中修改项目名称为中文, 其中想自定义html_title为<project> v<release> 文档, 
直接将html_title = u"<project> v<release> documentation"修改中文无法在页面上正确显示, 所以修改sphinx/config.py中line54为
