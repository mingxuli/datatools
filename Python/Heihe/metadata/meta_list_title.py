...
for group_id,uuid,meta,createdate,changedate in rows:
    ...
    #关键词
    ##主题关键词为:theme
   ## theme=root.getElementsByTagName('theme')
    ##for node in theme:
      ##  if node.hasChildNodes():            
           ## theme=theme[0].firstChild.data
        ##else:
           ## theme=u'没有英文标题'
    ##位置关键词为:place
    ##时间关键词为:temporal
    ...
    ##摘要
    idAbs=root.getElementsByTagName('idAbs')
    for node in idAbs:
        if node.hasChildNodes():
            idAbs=node.firstChild.data
        else:
            idAbs=''
    ##DOI
    ...
    ##空间范围
    westBL=root.getElementsByTagName('westBL')
    for node in westBL:
        if node.hasChildNodes():
            westBL=westBL[0].firstChild.data
        else:
            westBL=''
